...
import com.pcedu.blackbooks.entities.dto.FilterDTO;
...
public class BookRestController {
    ...
    public List<BookDTO> showListBook(@PathVariable List<Integer> ids) {
        ...
        for (Integer id : ids) {
            books.add(bookService.findByIdDTO(id));
        }
        return (books);
    }
...
//    }
    ...
    public List<BookDTO> showBooksByCategorieId(@PathVariable List<Integer> categorieIds) {
        ...
        for (Integer categorieId : categorieIds) {
            ...
            for (BookDTO book : bookService.findAllByCategorie(categorie)) {
                books.add(book);
            }
        }
        return (books);
    ...
    @CrossOrigin
    @PutMapping("/all/filters")
    public List<BookDTO> showBooksByFilters(@RequestBody FilterDTO filter) {
        List<BookDTO> books = new ArrayList<>();
        List<Book> allBooks = bookService.findAllBook();
        for (Book allBook : allBooks) {
            if (!filter.getAuthors().isEmpty()) {
                for (String author : filter.getAuthors()) {
                    if (Integer.parseInt(author) == allBook.getAuthorId().getId()) {
                        if (!filter.getCategories().isEmpty()) {
                            for (String categorieId : filter.getCategories()) {
                                for (CategorieBook list : allBook.getCategorieBookList()) {
                                    if (Integer.parseInt(categorieId) == list.getCategoryId().getId()) {
                                        books.add(new BookDTO(allBook));
                                    }
                                }
                            }
                        } else {
        ...
        return (noDuplicateBooks(books));
    }

    private List<BookDTO> noDuplicateBooks(List<BookDTO> books) {
...
//    }
    ...
    public ResponseEntity updateBook(@PathVariable int id, @RequestBody Book book) {
        if (bookService.update(updateCheck(book, bookService.findById(id)))) {
    ...
    private Book updateCheck(Book book, Book tempBook) {
        ...
        if (book.getNoofpages() != null) {
            tempBook.setNoofpages(book.getNoofpages());
        }
        if (book.getDescription() != null) {
            tempBook.setDescription(book.getDescription());
        }
        if (book.getRegularPrice() != null) {
            tempBook.setRegularPrice(book.getRegularPrice());
        }
        if (book.getStoreQuantity() != null) {
            tempBook.setStoreQuantity(book.getStoreQuantity());
        }
//        if(book.getImage() != null) tempBook.setImage(book.getImage());
        if (book.getTotalRating() != null) {
            tempBook.setTotalRating(book.getTotalRating());
        }
        if (book.getCountReviews() != null) {
            tempBook.setCountReviews(book.getCountReviews());
        }
        if (book.getCountSales() != null) {
            tempBook.setCountSales(book.getCountSales());
        }
        return (tempBook);
    }
}
